...
// ------------------------------------------------------------- photos ------
// Real photographs of each business, downloaded from the firm's blog and
// committed under public/img/listings. Mapping was taken from the blog's own
// post-title -> image pairing (see tools/fetch-listing-photos.mjs), not guessed
// from filenames. A listing with no entry here falls back to placeholder art.
const PHOTOS = {
...
const demoBrokers = BROKERS.map((b) => ({
  ...
  // Real headshot if we have one; otherwise an initials tile.
  photo_url: b.photo || avatar(b.name, 212), is_active: true, sort_order: b.sort,
}));
...
const demoListings = L.map((x) => {
  ...
  return {
    ...
    listing_images: PHOTOS[x.slug]
      ? [{ url: PHOTOS[x.slug], caption: x.city + ', ' + x.county, is_primary: true, sort_order: 0 }]
...
-- Listing photos point at files committed under public/img/listings.
...
select l.id, v.url, v.caption, v.is_primary, v.sort_order from l, (values
  (${q(PHOTOS[x.slug] || `https://placehold.co/1200x800/${HEX[x.category] || '1d6fb8'}/ffffff?text=${encodeURIComponent(x.category)}`)}, ${q(x.city + ', ' + x.county)}, true, 0)
...
// ---- photos-only SQL, for a database that already has the listings ----
// seed.sql only inserts images for listings it creates, so an existing project
// needs this to repoint the photos it already has.
let photoSql = `-- Point existing listing photos at the real images committed under
...
`;
for (const x of L) {
  if (!PHOTOS[x.slug]) continue;
  photoSql += `update public.listing_images i set url = ${q(PHOTOS[x.slug])}, caption = ${q(x.city + ', ' + x.county)}
  from public.listings l where i.listing_id = l.id and l.slug = ${q(x.slug)};
insert into public.listing_images (listing_id, url, caption, is_primary, sort_order)
select l.id, ${q(PHOTOS[x.slug])}, ${q(x.city + ', ' + x.county)}, true, 0
from public.listings l where l.slug = ${q(x.slug)}
  and not exists (select 1 from public.listing_images i where i.listing_id = l.id);

`;
}
writeFileSync(join(ROOT, 'supabase', 'listing_photos.sql'), photoSql);

// ---- restore SQL: re-insert listings deleted from a live database ----
// Figures are deliberately omitted (asking price, cash flow, gross revenue,
...
`;
  if (PHOTOS[x.slug]) {
    restoreSql += `insert into public.listing_images (listing_id, url, caption, is_primary, sort_order)
select l.id, ${q(PHOTOS[x.slug])}, ${q(x.city + ', ' + x.county)}, true, 0
from public.listings l where l.slug = ${q(x.slug)}
  and not exists (select 1 from public.listing_images i where i.listing_id = l.id);
`;
...
writeFileSync(join(ROOT, 'supabase', 'restore_listings.sql'), restoreSql);

console.log(`Wrote ${demoBrokers.length} brokers + ${demoListings.length} listings → demo-data.js, seed.sql, listing_photos.sql, restore_listings.sql`);
